...
if [ -z "$JRE" ] && [ -s "${CONFIG_HOME}/JetBrains/PyCharm2022.2/pycharm.jdk" ]; then
  USER_JRE=$(cat "${CONFIG_HOME}/JetBrains/PyCharm2022.2/pycharm.jdk")
...
if [ -n "$PYCHARM_VM_OPTIONS" ] && [ -r "$PYCHARM_VM_OPTIONS" ]; then
  ...
  elif [ -r "${CONFIG_HOME}/JetBrains/PyCharm2022.2/pycharm64.vmoptions" ]; then
    USER_VM_OPTIONS_FILE="${CONFIG_HOME}/JetBrains/PyCharm2022.2/pycharm64.vmoptions"
  fi
fi
